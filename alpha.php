...
<script>
    function bind_select_search_country(search_country, select, arr_name) {
        window[arr_name] = []
        $(select + " option").each(function(){
            window[arr_name][this.value] = this.text
        })
        $(search_country).keyup(function(e) {
            text = $(search_country).val()
            if (text != '' || e.keyCode == 8) {
                arr = window[arr_name]
        ...
        $(search_country).keydown(function(e) {
            if (e.keyCode == 8) // Backspace
                $(search_country).trigger('keyup')
        })

    }

    function bind_select_search_state(search_state, select, arr_name) {
        window[arr_name] = []
        $(select + " option").each(function(){
            window[arr_name][this.value] = this.text
        })
        $(search_state).keyup(function(e) {
            text = $(search_state).val()
            if (text != '' || e.keyCode == 8) {
                arr = window[arr_name]
                $(select + " option").remove()
                tmp  = ''
        ...
        $(search_state).keydown(function(e) {
            if (e.keyCode == 8) // Backspace
                $(search_state).trigger('keyup')
        })

    }

    $(document).ready(function() {
        bind_select_search_country('#search_country', '#country', 'options')
